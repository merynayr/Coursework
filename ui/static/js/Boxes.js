function getBoxes() {
    fetch('/box/select')
        ...
        .then(data => {
            ...
            data?.forEach(box => {
                tableHTML += `<tr><td>${box.Status ?? ''}</td><td>${box.Floor}${(box.BoxID).pad() ?? ''}</td><td>${box.Floor ?? ''}</td><td>${box.Area ?? ''}</td><td>№${box.Contract_id ?? ""} <font size="2">${formatDate(box.Contract_start) ?? ''} - ${formatDate(box.Contract_end) ?? ''}</font></td></tr>`;
            });
